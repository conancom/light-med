...
def main():
   ...
   # Enc at DO => CP-ABE 7/10 Policy Rate
   # Enc at Cloud  => CP-ABE 7/10 Poicy Rate
   # Dec at DU => CP-ABE 
   policyMain = 20

   numberOfPolicy = (policyMain*7)//10;
   if(policyMain == 1):
      numberOfPolicy = 1
   ...
   cpabe.reencrypt_key(symkeyname, policy) #CPABE Reencryption
   stop_cpabe_time = timeit.default_timer()

   cpabe_time = stop_cpabe_time - start_cpabe_time #Timer
   print("Cloud Enc Time (Reencrypt): " + str(cpabe_time))

   start_cpabe_dec_time = timeit.default_timer()
   cpabe.derecrypt(symkeyname, cpabe_keyName) #CPABE Decryption
   ...
   print("User Dec Time : " + str(cpabe_dec_time))

   '''
   numberOfPolicy = (numberOfPolicy*2)//3;
   policy = createPolicy(numberOfPolicy)
   ...
   numberOfPolicy = (numberOfPolicy*1)//3
   policy = createPolicy(numberOfPolicy)
